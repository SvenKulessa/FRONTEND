/**
 * CAPITAL AI — EVIDENCE BEFORE DECISION (AP-002)
 * Cryptographic SHA-256 Merkle Tree & Proof Generator
 */

// Pure TypeScript SHA-256 implementation for universal Browser & Node execution
...
export class MerkleTree {
  ...
  constructor(dataItems: any[]) {
    if (dataItems.length === 0) {
      throw new Error('MerkleTree requires at least one data item.');
    }

    // Hash each data item into a leaf
    this.leaves = dataItems.map((item) =>
      typeof item === 'string' ? sha256Sync(item) : sha256Sync(JSON.stringify(item))
    );
  ...
  private buildTree(): void {
    ...
    while (currentLayer.length > 1) {
      ...
      for (let i = 0; i < currentLayer.length; i += 2) {
        ...
        const parentHash = sha256Sync(left + right);
  ...
  public getProof(index: number): MerkleProof {
    ...
    const verified = MerkleTree.verifyProof(leaf, root, path);

    return { leaf, root, path, verified };
  }
  ...
  public static verifyProof(
    ...
    for (const step of path) {
      if (step.position === 'left') {
        computedHash = sha256Sync(step.hash + computedHash);
      } else {
        computedHash = sha256Sync(computedHash + step.hash);
      }
    }

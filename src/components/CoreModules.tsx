...
export const CoreModules: React.FC<CoreModulesProps> = ({ onSelectModule, onViewAllModules }) => {
  const renderModuleIcon = (type: string) => {
    switch (type) {
      case 'brain':
        return (
          <div className="w-10 h-10 rounded-full border border-amber-500/40 bg-amber-500/10 flex items-center justify-center text-[#F5B014] mb-3 shrink-0">
        ...
        {CORE_MODULES.map((module) => (
          ...
          >
            <div>
              ...
              {/* Title */}
              <h3 className="text-[14.5px] font-bold text-white tracking-tight mb-1.5 group-hover:text-amber-400 transition-colors">
                {module.title}
              </h3>
            ...
            {/* "Mehr erfahren ->" Link */}
            <div className="inline-flex items-center gap-1 text-[12px] font-semibold text-[#F5B014] group-hover:translate-x-0.5 transition-transform pt-2 border-t border-slate-800/60">
